...
# ---------------------------------------------------------------------------
# FORWARD VALIDATION HELPERS (module-level, no class dependency)
# ---------------------------------------------------------------------------

LOG_FILE    = "forward_validation_log.json"
RESULTS_DIR = "forward_validation_results"

def _fv_resolve_window(config: Dict) -> tuple:
    ...
    forward_end = date.today().strftime("%Y-%m-%d") if raw_end == "today" else raw_end
    return forward_start, forward_end

def _fv_check_min_window(forward_start: str, forward_end: str, min_days: int) -> bool:
    days = (pd.Timestamp(forward_end, tz="UTC") - pd.Timestamp(forward_start, tz="UTC")).days
    if days < min_days:
        print(f"\n  WARNING: Forward window is only {days} days "
              f"(minimum {min_days} required). "
              f"Come back in {min_days - days} days.")
        return False
    return True

def _fv_apply_gate(signals: List[SignalResult]) -> tuple:
    """Remove signals with confidence in [75, 80). Returns (filtered, n_removed)."""
    filtered = [s for s in signals if not (75 <= s.confidence < 80)]
    return filtered, len(signals) - len(filtered)

def _fv_extract_direction_metrics(trades: List) -> Dict:
    """Long/short split + 70-74 SHORT edge + 75-79 band metrics."""
    ...
    band_70_74_short = [t for t in trades if 70 <= t.final_confidence < 75 and t.direction == "SHORT"]
    band_70_74_long  = [t for t in trades if 70 <= t.final_confidence < 75 and t.direction == "LONG"]
    band_75_79       = [t for t in trades if 75 <= t.final_confidence < 80]
...
def _fv_compare_to_baseline(metrics: Dict, baseline: Dict) -> Dict:
...
def _fv_assess_readiness(
    ...
    if not baseline_cmp["sharpe_pass"]:
        issues.append(
            f"Sharpe {baseline_metrics.get('sharpe_ratio', 0):.3f} below floor "
            f"{baseline_cmp['sharpe_floor']:.3f} "
            f"(60% of baseline {config['baseline']['sharpe']:.3f})."
        )
    if not baseline_cmp["exp_pass"]:
        issues.append(
            f"Expectancy {baseline_metrics.get('expectancy', 0):.3f}R below floor "
            f"{baseline_cmp['exp_floor']:.3f}R "
            f"(60% of baseline {config['baseline']['expectancy']:.3f}R)."
        )
    ...
    if s_count >= 20:
        if s_exp < 0.10:
            issues.append(
                f"70-74 SHORT edge collapsed OOS: E {s_exp:.3f}R on {s_count} trades "
                f"(OOS baseline +0.458R). Edge is gone."
            )
        elif s_exp < 0.20:
            warnings.append(
                f"70-74 SHORT edge degrading: E {s_exp:.3f}R on {s_count} trades "
                f"(OOS baseline +0.458R). Monitor closely."
            )
    else:
        warnings.append(
            f"70-74 SHORT edge: only {s_count} OOS trades "
            f"(need 20+ for reliable read)."
        )

    max_dd = baseline_metrics.get("max_drawdown_pct", 0)
    if max_dd < -20.0:
        issues.append(f"Max drawdown {max_dd:.1f}% exceeds 20% threshold.")
    elif max_dd < -15.0:
        warnings.append(f"Max drawdown {max_dd:.1f}% is elevated. Monitor.")
    ...
    if gated_sharpe < baseline_sharpe - 0.3:
        warnings.append(
            f"75-79 gate is now HURTING OOS ({gated_sharpe - baseline_sharpe:.3f} Sharpe). "
...
def _fv_load_log(log_file: str) -> List[Dict]:
...
def _fv_append_log(log_file: str, entry: Dict):
    log = _fv_load_log(log_file)
...
def _fv_save_run_results(results_dir: str, run_date: str, data: Dict):
...
def _fv_print_metrics_block(label: str, metrics: Dict, direction: Dict,
                             cmp: Dict, baseline: Dict):
    print(f"\n  --- {label} ---")
...
def _fv_print_70_74_short_block(direction_baseline: Dict, direction_gated: Dict,
                                 config: Dict, log: List[Dict] = None):
    print(f"\n  --- 70-74 SHORT Edge (Highest Overfitting Risk) ---")
    print(f"  In-sample reference : 39 trades | WR 59.0% | E +1.064R")
    # Pull OOS baseline from most recent prior log entry
    if log:
        prior_entries = [e for e in log if e.get("run_date") != 
                        __import__('datetime').date.today().strftime("%Y-%m-%d")]
        if prior_entries:
            last = prior_entries[-1]
            d = last.get("direction_baseline", {})
            oos_label = f"OOS baseline ({last.get('run_date', 'prior run')})"
            oos_line  = (f"{d.get('band_70_74_short_count', '?')} trades | "
                        f"WR {d.get('band_70_74_short_wr', 0):.1f}% | "
                        f"E {d.get('band_70_74_short_exp', 0):+.3f}R")
        else:
            oos_label = "OOS baseline"
            oos_line  = "no prior runs in log"
    else:
        oos_label = "OOS baseline (Jun 2026)"
        oos_line  = "96 trades | WR 41.7% | E +0.458R"
    print(f"  {oos_label}: {oos_line}")
...
def _fv_print_75_79_gate_block(direction_baseline: Dict, direction_gated: Dict,
    ...
    print(f"  In-sample improvement : +0.798 Sharpe | OOS confirmed: {sharpe_delta:+.3f} Sharpe")
    print()
    ...
    if b_count < 10:
        ...
        print(f"\n  STATUS: Gate HELPING OOS (+{sharpe_delta:.3f} Sharpe). Confirmed effective.")
    elif sharpe_delta > -0.1:
        print(f"\n  STATUS: Gate NEUTRAL OOS ({sharpe_delta:+.3f} Sharpe). Continue monitoring.")
    else:
        print(f"\n  STATUS: Gate HURTING OOS ({sharpe_delta:+.3f} Sharpe). Consider disabling.")

def _fv_print_trend_summary(log: List[Dict]):
    if len(log) < 2:
        return
...
class MultiplierBacktest:
    ...
    def generate_oos_signals(self, forward_config: Dict) -> List[SignalResult]:
        """
        Generates GOOD signals in the OOS window only.
        Uses 'none' multiplier and the forward_config date range.
        Identical scan logic to generate_signals() - no divergence.
        """
        signal_generator = BacktestSignalGenerator(forward_config, "none")
        all_signals = []

        start_date = pd.Timestamp(forward_config["start_date"], tz="UTC")
        end_date   = pd.Timestamp(forward_config["end_date"],   tz="UTC")
        scan_times = pd.date_range(start=start_date, end=end_date, freq="4h", tz="UTC")
        ...
        print(f"  GOOD signals: {len(all_signals)} "
    ...
    def run_oos_validation(self):
        """
        Runs out-of-sample validation on the forward window (in_sample_end_date → today).
        Equivalent to run_forward_validation.py main() - merged here to keep
        everything in a single file.

        Called from main() when config["run_forward_validation"] is True.
        Set that flag to False for in-sample-only runs.
        """
        from datetime import date

        print("\n" + "=" * 80)
        print("FORWARD VALIDATION (OOS)")
        print("=" * 80)
        print(f"  Run date    : {date.today()}")
        print(f"  In-sample   : {self.config['start_date']} → "
              f"{self.config['in_sample_end_date']}")

        # --- Resolve window ---
        forward_start, forward_end = _fv_resolve_window(self.config)
        forward_days = (
            pd.Timestamp(forward_end,   tz="UTC") -
            pd.Timestamp(forward_start, tz="UTC")
        ).days
        print(f"  Forward     : {forward_start} → {forward_end} ({forward_days} days)")
        print(f"  Baseline    : {self.config['baseline']['source']}")
        print(f"               Sharpe {self.config['baseline']['sharpe']} | "
              f"E {self.config['baseline']['expectancy']}R | "
              f"WR {self.config['baseline']['win_rate']}%")

        # --- Minimum window check ---
        min_days = self.config.get("forward_min_days", 45)
        if not _fv_check_min_window(forward_start, forward_end, min_days):
            print("\n  Skipping OOS validation - window too short.")
            return

        # --- Build forward config (scoped to OOS window only) ---
        forward_config = {**self.config,
                          "start_date": forward_start,
                          "end_date":   forward_end}

        # --- Data fetch: extend CSV cache to forward_end ---
        print(f"\n  Refreshing data cache to {forward_end}...")
        for symbol in forward_config["symbols"]:
            df = self.data_loader.fetcher.get_historical_data(
                symbol        = symbol,
                interval      = "1h",
        ...
        # Rebuild data_loader so the refreshed CSVs are picked up
        self.data_loader = HistoricalDataLoader(forward_config)

        # --- Generate signals ---
        print(f"\n  Generating OOS signals...")
        all_signals = self.generate_oos_signals(forward_config)

        if not all_signals:
            print("\n  No signals in forward window. Skipping OOS validation.")
            return

        # --- Apply 75-79 gate ---
        gated_signals, n_removed = _fv_apply_gate(all_signals)
        print(f"  75-79 gate  : {n_removed} signals removed "
              f"({len(gated_signals)} remaining)")

        # --- Simulate trades ---
        print(f"\n  Simulating baseline (no gate)...")
        baseline_trades = self.simulate_trades(sorted(all_signals,     key=lambda s: s.timestamp))
        print(f"  Baseline trades: {len(baseline_trades)}")

        print(f"\n  Simulating gated (75-79 excluded)...")
        gated_trades = self.simulate_trades(sorted(gated_signals, key=lambda s: s.timestamp))
        print(f"  Gated trades   : {len(gated_trades)}")

        if not baseline_trades:
            print("\n  No trades simulated in forward window.")
            return

        # --- Metrics ---
        baseline_metrics = self.metrics_calculator.calculate_metrics(
            baseline_trades, "OOS_Baseline"
        )
        gated_metrics = (
            self.metrics_calculator.calculate_metrics(gated_trades, "OOS_Gated")
            if gated_trades
            else {"error": "No gated trades"}
        )

        direction_baseline = _fv_extract_direction_metrics(baseline_trades)
        direction_gated    = _fv_extract_direction_metrics(gated_trades) if gated_trades else {}

        baseline_ref = self.config["baseline"]
        baseline_cmp = _fv_compare_to_baseline(baseline_metrics, baseline_ref)
        gated_cmp    = (
            _fv_compare_to_baseline(gated_metrics, baseline_ref)
            if gated_trades else {}
        )

        # --- Cumulative OOS trade count ---
        log = _fv_load_log(LOG_FILE)
        run_date_str = date.today().strftime("%Y-%m-%d")
        prior_trades = sum(
            e.get("baseline_metrics", {}).get("total_trades", 0)
            for e in log
            if e.get("run_date") != run_date_str
        )
        total_oos_trades = prior_trades + len(baseline_trades)
        ...
        print("=" * 80)
        _fv_print_70_74_short_block(direction_baseline, direction_gated, self.config, log=log)

        print(f"\n{'='*80}")
        print("75-79 BAND GATE EFFECTIVENESS")
        print("=" * 80)
        if "error" not in gated_metrics:
            _fv_print_75_79_gate_block(
        ...
        print("=" * 80)
        print(f"\n  Cumulative OOS trades: {total_oos_trades} "
              f"/ {self.config['min_oos_trades_for_deployment']} required")

        verdict, reason, issues, warnings_list = _fv_assess_readiness(
            baseline_metrics   = baseline_metrics,
            gated_metrics      = gated_metrics if "error" not in gated_metrics else {},
            direction_baseline = direction_baseline,
            baseline_cmp       = baseline_cmp,
            total_oos_trades   = total_oos_trades,
            min_oos_trades     = self.config["min_oos_trades_for_deployment"],
            config             = self.config,
        )
        ...
        run_entry = {
            ...
            "baseline_metrics":   {k: v for k, v in baseline_metrics.items()
                                   if not k.startswith("_")},
            "gated_metrics":      {k: v for k, v in gated_metrics.items()
                                   if not k.startswith("_")}
                                  if "error" not in gated_metrics else {},
            "direction_baseline": direction_baseline,
        ...
        _fv_append_log(LOG_FILE, run_entry)
        _fv_save_run_results(RESULTS_DIR, run_date_str, run_entry)

        print(f"\n  Run complete. Next run recommended in 1-2 weeks.")
        print(f"  OOS trades still needed: "
              f"{max(0, self.config['min_oos_trades_for_deployment'] - total_oos_trades)}")
        print("=" * 80)
...
def main():
    ...
    if BACKTEST_CONFIG.get("run_forward_validation", False):
        backtest.run_oos_validation()
    else:
        print("\n  [OOS validation skipped - set run_forward_validation=True in config to enable]")
